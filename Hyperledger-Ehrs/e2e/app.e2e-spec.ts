...
describe('Starting tests for Hyperledger-Ehrs', function() {
  ...
  it('website title should be Hyperledger-Ehrs', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Hyperledger-Ehrs');
    })
  });
  ...
  it('navbar-brand should be Hyperledger-Ehrs',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Hyperledger-Ehrs');
    });
  });
